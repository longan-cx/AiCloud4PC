...
BOOL CFileDialogEx::ShowOpenFileDlg(CDuiString lpszInitialDir)
{
	//m_stOFN.lpstrInitialDir = lpszInitialDir;
//	m_stOFN.lpstrInitialDir = _T("C:/Users/cyc/Pictures"); ??????????

	lpszInitialDir.Replace(_T("/"), _T("\\"));
	m_stOFN.lpstrInitialDir = lpszInitialDir;

	return ::GetOpenFileName(&m_stOFN);
}

BOOL CFileDialogEx::ShowSaveFileDlg(CDuiString lpszInitialDir)
{
	lpszInitialDir.Replace(_T("/"), _T("\\"));
	m_stOFN.lpstrInitialDir = lpszInitialDir;
